...
class PinholeMemImgReader : public PinholeCamera
{
public:
	using Ptr = std::shared_ptr<PinholeMemImgReader>;
	 ...
	 */
	static PinholeMemImgReader::Ptr Create(const cv::Mat1d& CameraMatrix = cv::Mat1d(), const cv::Mat1d& DistCoeff = cv::Mat1d());
public:
	...
	bool addImage(cv::InputArray Img);
	bool addImages(cv::InputArrayOfArrays Imgs);
	int getBufferSize();
