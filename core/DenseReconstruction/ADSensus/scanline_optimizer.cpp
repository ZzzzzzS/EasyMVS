...
void ScanlineOptimizer::ScanlineOptimizeLeftRight(const float32* cost_so_src, float32* cost_so_dst, bool is_forward)
{
	...
	for (sint32 y = 0u; y < height; y++) {
		...
		std::vector<float32> cost_last_path(disp_range + 2, ADCCensus::Large_Float);
		...
		// 路径上上个像素的最小代价值
		float32 mincost_last_path = ADCCensus::Large_Float;
		...
		for (sint32 j = 0; j < width - 1; j++) {
			...
			uint8 d2 = d1;
			float32 min_cost = ADCCensus::Large_Float;
