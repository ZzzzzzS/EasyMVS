...
{
	qDebug()<<"Scroll Scale about to close";
...
{
    QPixmap picture = QPixmap::fromImage(Picture);
	...
	case DrawingLine:
		{
			...
			qDebug() << Begin << End;
			//emit this->DrawRectangleSignal(Begin, End);
			emit this->DrawLineSignal(Begin, End);
			this->DrawStatus = DrawStatus_e::NoDrawing;
			break;
		}
	case DrawingCircle:
...
void ScrollScale::mouseDoubleClickEvent(QMouseEvent * event)
{
	if (AllowPopOut == true && OriginalPicture.isNull() == false)
	{
		...
		QObject::connect(Window, &ScrollScale::DrawLineSignal, [this](QPoint Begin, QPoint End) {
			qDebug()<<"Drawline: begin:"<<Begin<<"end:"<<End;
			emit this->DrawLineSignal(Begin, End);
		});
...
void ScrollScale::paintEvent(QPaintEvent * event)
{
	...
	case DrawingLine:
		DrawingPainter.begin(this);
		DrawingPainter.setPen(this->DrawingPen);
		DrawingPainter.drawLine(ClickedPointOnScreen.x(), ClickedPointOnScreen.y(), MovePointOnScreen.x()+ClickedPointOnScreen.x(), MovePointOnScreen.y()+ClickedPointOnScreen.y());
		//DrawingPainter.drawRect(ClickedPointOnScreen.x(), ClickedPointOnScreen.y(), MovePointOnScreen.x(), MovePointOnScreen.y());
		DrawingPainter.end();
		break;
	case DrawingCircle:
...
void ScrollScale::InitContextMenu()
{
	...
	this->DrawLineAction = new QAction(tr("Draw Line"), this);
	...
	this->addAction(this->DrawLineAction);
	...
	QObject::connect(this->DrawLineAction,&QAction::triggered,[this]() {
		this->DrawStatus = DrawStatus_e::WillDrawLine;
		qDebug() << "Will Line";
	});
...
ScrollScaleWithIndex::ScrollScaleWithIndex(const int index,QWidget *parents)
	:ScrollScale(parents),
	Index(index)
{
}

void ScrollScaleWithIndex::mousePressEvent(QMouseEvent *event)
{
	ScrollScale::mousePressEvent(event);
	qDebug()<<"Index:"<<Index;
	emit this->ImgClicked(this->Index);
}